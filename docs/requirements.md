...
- GitHub Actions scheduling latency is acceptable for hourly collection.
- RSS, Atom, JSON Feed, GitHub release feeds, and podcast RSS are sufficient initial source types.
...
- Copilot CLI is a viable GitHub-native automation path when authenticated with a GitHub account that has a valid Copilot license and a fine-grained token with Copilot Requests permission.
...
- CI workflow for linting, tests, type checks, build checks, and contract validation.
...
Default to static HTML, CSS, TypeScript, Web Components, and a Service Worker. A framework is acceptable only if it adds clear value for design or functionality while keeping dependencies low, stable, and well-known.
...
| Frontend stack | Prefer vanilla TypeScript/Web Components; use a framework only if it adds clear value with low dependency overhead. |
