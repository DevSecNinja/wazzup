...
class PwaAssetTests(unittest.TestCase):
    ...
    def test_pwa_tracks_seen_briefing_items_locally(self) -> None:
        ...
        self.assertIn("const SEEN_READ_LINE_RATIO = 0.25", app)
        self.assertIn("const SEEN_DWELL_MS = 1500", app)
        self.assertIn("let pauseSeenScansUntilInput = false", app)
        self.assertIn("function bulletHasReachedReadLine(bulletEl)", app)
        self.assertIn("window.innerHeight * SEEN_READ_LINE_RATIO", app)
        self.assertIn("let readLineMatchFound = false", app)
        self.assertIn("window.addEventListener('scroll', scan, { passive: true })", app)
        self.assertIn("window.addEventListener('wheel', resumeAfterInput, { passive: true })", app)
        self.assertIn("window.addEventListener('resize', scan)", app)
        self.assertIn("scheduleBriefingBulletSeen", app)
        self.assertIn("cancelBriefingBulletSeen", app)
        self.assertIn("bindHideSeenButton(seenState)", app)
        self.assertIn("observeBriefingItems(seenState);", app)
