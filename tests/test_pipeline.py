    ...
    def test_generate_static_data_from_fixtures(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                self.assertEqual(1, run_status["schemaVersion"])
                self.assertEqual("degraded_sources", run_status["status"])
                self.assertEqual("degraded", run_status["sourceStatus"])
                self.assertEqual("ok", run_status["providerStatus"])
